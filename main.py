...
class MainProgram:
    ...
    def __init__(self, LOGGER=None):
        ...
        self.LOG.info(f"Configs file {os.path.join(USERDATA_DIR, CONFIGS)} loaded.")
    ...
    def load_configs(self):
        """ reads userdata/configs.json & stores it here while we run"""
        with open(os.path.join(USERDATA_DIR, CONFIGS), 'r') as file:
    ...
    def launch_oauth_protocol(self, code_params={}):
        ...
        if code_params.keys():
            ...
            self.LOG.info(f"using params set in {CONFIGS} to perform oauth... ")
            ...
            self.LOG.info(f"TOKEN: {TOKEN_POST_URL}")
